  ...
  onPointerMove = (event) => {
    if (event.pointerType == 'touch' && this._input != INPUT.CURSOR) {
      switch (this._input) {
        ...
        case INPUT.ONE_FINGER_SWITCHED:
          const movement =
            this.calculatePointersDistance(this._touchCurrent[0], event) * this._devPxRatio;
    ...
    if (this._downValid) {
      const movement =
        this.calculatePointersDistance(this._downEvents[this._downEvents.length - 1], event) *
        this._devPxRatio;
      ...
      if (this._downValid) {
        ...
        if (downTime <= this._maxDownTime) {
          if (this._nclicks == 0) {
            ...
            const movement =
              this.calculatePointersDistance(this._downEvents[1], this._downEvents[0]) *
              this._devPxRatio;
  ...
  onRotationAnim = (time, rotationAxis, w0) => {
    ...
    if (this._state == STATE.ANIMATION_ROTATE) {
      ...
      if (w > 0) {
        ...
        this._angleCurrent =
          0.5 * -this.dampingFactor * Math.pow(deltaTime, 2) + w0 * deltaTime + 0;
      ...
      if (delta >= 0) {
        ...
        if (angle >= 45) {
          ...
          const rayLength = Math.sqrt(
            Math.pow(this._v2_1.x, 2) + Math.pow(cameraGizmoDistance - this._v2_1.y, 2),
          );
